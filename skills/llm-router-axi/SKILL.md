...
npx -y llm-router-axi check --harness opencode --model opencode-go/qwen3.8-flash
...
npx -y llm-router-axi shadow report
```
...
`~/.local/state/llm-router-axi/shadow-ledger.jsonl`. The decision never changes; the hook
