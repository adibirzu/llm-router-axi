...
- `check --harness H --model M` is the explicit-override gate: it resolves the matching policy candidate group, reuses selector and spawn-capacity refusals, returns the next eligible candidate (which must pass the same capacity verdict), and does not rotate least-recent-use state. `--force-override` appends, only for an override that would otherwise be refused, a credential-free audit row to `override-audit.jsonl` (`LLM_ROUTER_OVERRIDE_LOG` overrides the path in tests).
