...
- Jev slice 1 (`src/jev/`, `classify`, `doctor`) only classifies and never affects `route`: `classify` is the task-descriptor classifier while `classify-evidence` stays the depletion detector — do not re-merge them. The TypeSafe key comes ONLY from `TYPESAFE_API_KEY` (never a flag) and must never appear in output, errors, logs, or fixtures (`test/jev.test.ts` enforces this with a sentinel). `src/jev/client.ts` lists the documented-contract gaps.
