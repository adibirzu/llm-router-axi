...
- This repo is the P2 **implementation** of the `axi-router-program`: policy schema/validator plus live `route`/`explain`/`record`. Selection/ranking/fallback must stay at parity with the firstmate selector (`src/selector.ts` is the port; `test/parity/selector-parity.test.ts` runs the pinned selector side by side). See [docs/design.md](docs/design.md).
...
- Telemetry comes from `usage-axi --json --full` (override the binary with `LLM_ROUTER_USAGE_AXI`) or `--usage-json`; provider identity and pool pricing are resolved in `src/usage.ts`. State (cooldown + least-recent-use ledger) lives under `~/.local/state/llm-router-axi` (`LLM_ROUTER_STATE_FILE` overrides the file).
